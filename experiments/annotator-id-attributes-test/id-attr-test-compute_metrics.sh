...
for config_path in experiments/annotator-id-attributes-test/*.json; 
do
    echo Computing metrics for $config_path
    path_no_ending=$(echo $config_path | cut -d\. -f1)
    python3 -m multi_annotator.metrics $config_path $path_no_ending.results.csv --do_calculate_cis=True
done
